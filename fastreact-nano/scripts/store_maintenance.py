...
def compact_store(store: StoreService, keep_last: int, make_backup: bool) -> dict[str, Any]:
    backup_path = backup_store(store.root) if make_backup else None
    before = store.stats()
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.command == "compact":
        result = compact_store(store, keep_last=args.keep_last, make_backup=not args.no_backup)
